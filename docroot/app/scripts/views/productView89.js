...
(function() {
    ...
    Webshop.Views.ProductView = Backbone.View.extend({
        ...
        render: function(data) {
            var self = this;
                ...
                var currEventDate = $('#FnyFirstRecord').data('firstrecord');
                 ...
                 /// if event not started then active this code
            ...
            if (this.liveInventory.clear().set(data)) {
                this.liveInventory.fetch({
                    ...
                    success: function(model, response) {
                        ...
                        var oldPrice = "INR" + " " + response.sellerSKUs.mrp,
                            newPrice = "INR" + " " + response.product.productVariants.sellerSKUs.mrp;

                        currSKUQnty = response.product.productVariants.sellerSKUs.inventory;
                        currSKUMaxAllowedQnty = response.product.productVariants.sellerSKUs.maxQuantity;
                        selectedInventoryId = response.liveinventory.eventInventoryId;
                        selectedErpSku = response.liveinventory.erpSku;
                        selectedEventId = response.product.productVariants.sellerSKUs.event.eventId;
        ...
        updateSelectedSKU: function(event, data) {
            event.preventDefault();
            ...
            var skuList, oldPrice,newPrice, skuinventory, attrName, attrDesc, description, description1, description2, description3;

            $('.select-choice .tiles a').click(function(e){
                e.preventDefault();
                skuList = $(this).html();
                ...
                $.each(data.product.productVariants, function(index, value){
                        ...
                        if(skuList.toLowerCase() === value.variantCode.toLowerCase())
                                {  
                                  ...
                                  $('.productDescription .attrName').text(attrName);
                                  $('.productDescription .attrDesc').text(attrDesc);
        ...
        updateProductGallery: function(data) {
            ...
            var prodGalleryThumbList = "";           
             ...
             for(i=0;i < data.product.images.length;i++){
                prodGalleryThumbList += '<div class="slide"><a href="'+data.product.images[i].originalUrl+'" class="thumbnail-image" rel="zoom-id:ex1"  rev="'+data.product.images[i].detailsURL+'" data-image-detail=" ' ;
                prodGalleryThumbList += /*imageServerBaseURL +*/ data.product.images[i].detailUrl;
                prodGalleryThumbList += '"  data-image-original="';
                prodGalleryThumbList += /*imageServerBaseURL +*/ data.product.images[i].originalUrl;
                prodGalleryThumbList += '"><img src="';
                prodGalleryThumbList += /*imageServerBaseURL +*/ data.product.images[i].thumbUrl;
