...
(function() {
    ...
    Webshop.Views.OrderHistoryView = Backbone.View.extend({
		...
		events : {
			...
			'click #cancelOrder' : 'renderCancelItemsFancyBox',
			 ...
			 $.ajax({
	          ...
	          success: function(data) {
				if(data.domainResponse.responseCode === 'SUCCESS'){
					...
					var cancelOrderStatus = data.domainResponse.entitiesResponse[0].baseDTO.orderLines[0].showCancelButton;											
						
						if(data.domainResponse.entitiesResponse[0].baseDTO.orderLines.sellerSku)
						{
							erSKU = data.domainResponse.entitiesResponse[0].baseDTO.orderLines.sellerSku;
						}else{
							erSKU = data.domainResponse.entitiesResponse[0].baseDTO.orderLines.erpSku;
						}
						var htmlData =  '<tr><td>1</td><td>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines.productName+
							...
							'"/></td><td>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines.statusMessage+
							'<br><a data-orderno="'+dataOrderno+'"  data-awb-no="" data-orderline-id="'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines.orderLineId+'" id="trackMyOrder" class="fancybox '+data.domainResponse.entitiesResponse[0].baseDTO.status+'" href="#ordernum1">Track Item</a><br><br><a data-orderno="'+dataOrderno+'"  data-awb-no="" data-orderline-id="'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines.orderLineId+'" id="cancelOrder"  class="fancybox '+data.domainResponse.entitiesResponse[0].baseDTO.status+'" href="#ordernum1">Cancel Order</a></td>
							<td>'+"Rs." +data.domainResponse.entitiesResponse[0].baseDTO.inboundOrderLines.mrp+
							'</td><td>'+"Rs." +data.domainResponse.entitiesResponse[0].baseDTO.inboundOrderLines.salePriceExclVat+
							'</td><td>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines.quantity+
							'</td><td>'+"Rs." +parseInt(data.domainResponse.entitiesResponse[0].baseDTO.orderLines.quantity * data.domainResponse.entitiesResponse[0].baseDTO.inboundOrderLines.salePriceExclVat)+'</td></tr>';
							
							$('.orddetail-table tr:first-child').after(htmlData);

							if(cancelOrderStatus.toString() == "true"){
								$('.orddetail-table #cancelOrder').show();
							}else{
								$('.orddetail-table #cancelOrder').hide();
							}
							...
							var cancelOrderStatus =obj.showCancelButton;

							console.log("cancelOrderStatus = " + JSON.stringify(cancelOrderStatus))	;
								...
								if(obj.sellerSku){
									...
									var htmlData =  '<tr><td>'+(index +1)+'</td><td>'+obj.productName+
													...
													'"/></td><td>'+obj.statusMessage+
													'<br><a data-awb-no="" data-orderno="'+dataOrderno+'" data-orderline-id="'+obj.orderLineId+'" id="trackMyOrder" class="fancybox '+data.domainResponse.entitiesResponse[0].baseDTO.status+'" href="#ordernum1" >Track Item</a><br><a data-awb-no="" data-orderno="'+dataOrderno+'" data-orderline-id="'+obj.orderLineId+'" id="cancelOrder" class="fancybox '+data.domainResponse.entitiesResponse[0].baseDTO.status+'" href="#ordernum1" >Cancel Order</a></td>
													<td>'+"Rs."+obj.mrp+'</td>
													<td>'+"Rs."+obj.salePriceExclVat+'</td><td>'+obj.quantity+
													'</td><td>'+"Rs."+parseFloat(obj.salePriceExclVat * obj.quantity)+'</td></tr>';
										    

										$('.orddetail-table tr:last-child').after(htmlData);
										if(cancelOrderStatus.toString() == "true"){
												$('.orddetail-table tr:last-child #cancelOrder').show();
											}else{
												$('.orddetail-table tr:last-child #cancelOrder').hide();
											}
						 ...
						 var customerInfoData = '<tr><td style="width:33% !important"><table class="customerBillingInfo"><tr><td class="custHeading">CUSTOMER INFORMATION : </td></tr><tr><td><strong>'+data.domainResponse.entitiesResponse[0].baseDTO.customer.firstName+ ' ' +data.domainResponse.entitiesResponse[0].baseDTO.customer.lastName+
						     ...
						     '</td></tr></table></td><td style="width:33% !important" ><table class="customerBillingInfo" ><tr><td class="custHeading"> SHIPPING INFORMATION:</td></tr><tr><td>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines[0].shippingAddress.fullName+
						     '</td></tr><tr><td>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines[0].shippingAddress.address1+'</br>'+data.domainResponse.entitiesResponse[0].baseDTO.orderLines[0].shippingAddress.address2+
						     '</td></tr></table></td><td style="width:33% !important"><table><tr><td><strong>Cart Total</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalBasePrice+'<tr><td>';
						     if(data.domainResponse.entitiesResponse[0].baseDTO.youSavedAmount){
						      customerInfoData = customerInfoData +'<tr><td><strong>Promo/Coupon (-)</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.youSavedAmount+'</td></tr>'
						     }
						     if(data.domainResponse.entitiesResponse[0].baseDTO.totalStoredCredit){
						      customerInfoData = customerInfoData +'<tr><td><strong>Credits </strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalStoredCredit+'</td></tr>'
						     }
						     if(data.domainResponse.entitiesResponse[0].baseDTO.totalShippingCharge){
						      customerInfoData = customerInfoData +'<tr><td><strong>Delivery Charges</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalShippingCharge+'</td></tr>'
						     }
						     if(data.domainResponse.entitiesResponse[0].baseDTO.totalCodCharge){
						      customerInfoData = customerInfoData +'<tr><td><strong>COD Charges</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalCodCharge+'</td></tr>'
						     }
						     if(data.domainResponse.entitiesResponse[0].baseDTO.totalTax){
						      customerInfoData = customerInfoData +'<tr><td><strong>Taxes</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalTax+'</td></tr>'
						     }
						     customerInfoData = customerInfoData +'<tr><td><strong>Net Payable Amount</strong>'+data.domainResponse.entitiesResponse[0].baseDTO.totalAmountPaid+
						      '</td></tr></table></td></tr>'  
		...
		renderCancelItemsFancyBox :  function(e){
			e.preventDefault();
			...
			this.orderCancelView = new Webshop.Views.orderCancelView();
			this.orderCancelView.renderCancelDetailsView(data);
			//console.log("orderno is --"+orderNo);
